...
public class Camera2BasicFragment extends Fragment
    ...
    // 提示目前IP
    private  TextView IPreminder;
    // IP地址用字符串表示
    String IPstring;
    // IP地址
    InetAddress address;
    // 是否获得了IP地址
    boolean has_got_valid_IP = false;
    ...
    // 发送图片数据，接收信息
    private void sendAndRecv() throws IOException{
        ...
        switch(Integer.parseInt(serverBack)) {
            case 1: mePlayer = MediaPlayer.create(getActivity(), R.raw.yi1); break;
            case 2: mePlayer = MediaPlayer.create(getActivity(), R.raw.er4); break;
            case 3: mePlayer = MediaPlayer.create(getActivity(), R.raw.san1); break;
            case 4: mePlayer = MediaPlayer.create(getActivity(), R.raw.si4); break;
            case 5: mePlayer = MediaPlayer.create(getActivity(), R.raw.wu3); break;
    ...
    private static final String TAG = "DataCommDemo";
    ...
    public void onResume() {
        ...
        confirmButton.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                // 检查是否有效IP
                if(isIPAddressByRegex(IPstring)) {
                    try {
                        address = InetAddress.getByName(IPstring);
                    }
                    catch (UnknownHostException e) {
                    }
                    has_got_valid_IP = true;
                    // 提示当前IP，重绘文本
                    IPreminder.setText("当前目的IP为：" + IPstring);
                    IPreminder.invalidate();
                }
                // IP不合法，提示重新输入
                else
    ...
    private void lockFocus() {
        // 下面的操作使得相机只有在“极佳”条件下才会拍照，我们不用这么苛刻，否则拍照键可能无响应
        /*
        try {
